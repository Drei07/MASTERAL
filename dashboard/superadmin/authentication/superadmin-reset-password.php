...
if(isset($_GET['id']) && isset($_GET['code']))
{
 ...
 $stmt = $superadmin->runQuery("SELECT * FROM superadmin WHERE superadmin_id=:uid AND tokencode=:token");
 ...
 if($stmt->rowCount() == 1)
 {
  if(isset($_POST['btn-update-password']))
  {
   $npass = $_POST['new-password'];
    ...
    $npass = md5($npass);
    $stmt = $superadmin->runQuery("UPDATE superadmin SET password=:upass, tokencode=:token WHERE superadmin_id=:uid");
    $stmt->execute(array(":token"=>$code,":upass"=>$npass,":uid"=>$rows['superadmin_id']));
    
    $_SESSION['status_title'] = "Success !";
    $_SESSION['status'] = "Password is updated. Redirecting to Sign in.";
    $_SESSION['status_code'] = "success";
    header("refresh:2;../../../public/superadmin/signin");
