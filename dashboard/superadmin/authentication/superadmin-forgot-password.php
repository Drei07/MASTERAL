...
// localhost
//URL
$main_url = "http://localhost/MASTERAL";
...
if(isset($_POST['btn-forgot-password']))
{
 ...
 $stmt = $superadmin->runQuery("SELECT superadmin_id, tokencode FROM superadmin WHERE email=:email; AND tokencode=:code LIMIT 1");
 $stmt->execute(array(":code"=>$code,":email"=>$email));
 $row = $stmt->fetch(PDO::FETCH_ASSOC); 
 if($stmt->rowCount() == 1)
 {
  $id = base64_encode($row['superadmin_id']);
  $code = ($row['tokencode']);
